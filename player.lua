...
function Player:handleInput(delta_time)
	...
	if love.keyboard.isDown("left") and not love.keyboard.isDown("right") then
		self.velocity.speedX = self.velocity.speedX - self.velocity.delta * delta_time
	...
	if love.keyboard.isDown("up") and not love.keyboard.isDown("down") then
		self.velocity.speedY = self.velocity.speedY - self.velocity.delta * delta_time
	...
	self.x = math.min(math.max(self.x + self.velocity.speedX, 0), screen.width - self.width)
	self.y = math.min(math.max(self.y + self.velocity.speedY, 0), screen.height - self.height)
